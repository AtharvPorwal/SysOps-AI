...
def create_and_run_container(image_name=""):
...
def run_command_in_container(container, command):
    try:
        exec_id = container.client.api.exec_create(container.id, command, tty=True)["Id"]
        output_stream = container.client.api.exec_start(exec_id, stream=True, tty=True)
